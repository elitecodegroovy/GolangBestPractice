...
func DefineNameTemplate() {
	t, err := template.New("test").Parse(`{{define "T"}}Golang , {{.}}!{{end}}`)
	err = t.ExecuteTemplate(os.Stdout, "T", "Concurrent Programming!")
	if err != nil {
...
func DefineTemplate() {
	...
	note := Note{"text/templates", "Template generates textual output", time.Now()}
